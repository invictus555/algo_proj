...
type SinglyLinkedList[T interface{}] struct {
	size    int
	sortOpt utils.SortOpt
	cmpFunc api.CompareFunc[T]
	header  *SinglyLinkedListNode[T]
}

func NewSinglyLinkedList[T interface{}](sortOpt utils.SortOpt, cmpFunc api.CompareFunc[T]) api.List[T] {
...
func (list *SinglyLinkedList[T]) Insert(val T) bool {
	...
	if list.cmpFunc != nil && list.sortOpt != utils.No { // 以升序/降序插入
		cmpfunc := list.cmpFunc
		curPointer := head.next
		for ; curPointer != nil; curPointer = curPointer.next {
			// 升序插入需找最近比val小的位置
			if list.sortOpt == utils.Asc && cmpfunc(curPointer.val, val) <= 0 { // next.val <= val
				targetPosition = curPointer // 记录最后一个次匹配结果所在的指针
				continue
			}
			// 降序插入需找最近比val大的位置
			if list.sortOpt == utils.Desc && cmpfunc(curPointer.val, val) >= 0 { // next.val >= val
				targetPosition = curPointer // 记录最后一个次匹配结果所在的指针
				continue
			}
			break
